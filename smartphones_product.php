<?php
session_start();
include 'init.php'; // Include inițializările necesare

// Setează Bearer token-ul și endpoint-urile
// Include token și apiUrl dintr-un fișier extern
include('./key.php'); 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add_review') {
    ...
    // Pregătește datele pentru adăugarea recenziei
    $data = [
        'data' => [
    ...
    header("Location: " ."http://localhost/Halley/smartphones_product.php?id=$product_id");
...
$response = makeRequest($productUrl, $token, 'GET');
...
// Calcularea ratingului mediu
$averageRating = $totalReviews > 0 ? round($totalRating / $totalReviews, 1) : 0;

// Poți continua să afișezi detaliile produsului, recenziile și alte informații aici
